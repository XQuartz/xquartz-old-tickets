...
# $Id: stereo.tcl,v 1.10 2009/02/07 07:04:50 gregcouch Exp $
...
proc stereo::makeGraphics {mode} {
	...
	if { [catch { togl $name -width $width -height $height -rgba true -stereo "$mode" -double true -depth true -sharelist main -create create_cb -display display_cb -reshape reshape_cb -eyeseparation 0.05 -convergence 2.0 } error] } {
		pack forget $stereo::oldname
		...
		if { "$stereo::oldname" != "" } {
			destroy $stereo::oldname
		}
		set stereo::oldname $name
