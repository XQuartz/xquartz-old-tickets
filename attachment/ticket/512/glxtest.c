...
static GLXFBConfig choose_fbconfig(Display *dpy, int scr, PIXELFORMATDESCRIPTOR *pfd) {
    ...
    return list[bestFormat].fbconfig;
}
...
int main(void) {
    ...
    fbc = fbcs[0];
#else
	fbc = choose_fbconfig(dpy, scr, &pfd);
	if (!fbc) {
		fprintf(stderr, "No suitable FBConfigs!\n");
		XCloseDisplay(dpy);
		return 1;
...
#endif
    vis = glXGetVisualFromFBConfig(dpy, fbc);
    ...
    glc = glXCreateContext(dpy, vis, None, True);
    if (!glc) {
